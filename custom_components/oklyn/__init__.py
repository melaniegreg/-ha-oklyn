...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    client = OklynClient(session, entry.data[CONF_API_TOKEN], entry.data[CONF_DEVICE_ID])
    coordinator = OklynDataUpdateCoordinator(hass, client, entry.options.get(CONF_SCAN_INTERVAL, entry.data.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)))
    try:
